...
import numpy as np
from model import *
import config as c # import the config value 
import datasets
...
data_loader = datasets.testloader
num_image = len(data_loader)



with torch.no_grad():
    for i, data in zip(trange(num_image),data_loader):
        data = data.to(device)
